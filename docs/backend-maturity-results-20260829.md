...
This is the preserved historical report from the old validator, not validation
of the current script. Its overall success and LiteInst B1.5 award below were
incorrect: DBT B1.5 failed, and LiteInst's process-tree check timed out. The
current validator requires every measured row, including that process-tree
check, to pass. It also uses one shared fresh release target; the historical
claim of independent targets below was inaccurate. These original observations
remain unchanged so the correction can be audited. A fresh full run is needed
before reporting current maturity.
