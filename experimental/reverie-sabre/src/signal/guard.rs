...
impl<T> GuardedSequencer<T> {
    ...
    pub fn invoke(&self, handler: fn(T), argument: T) -> bool {
        ...
        // Standard (non-realtime) signals may be coalesced. If the bounded
        // queue is full, restore the counters instead of panicking from signal
        // context and taking down the guest.
        if self.queue.enqueue((handler, argument)).is_err() {
            self.guard_state
                .fetch_sub(QUEUED_COUNT_UNIT + GUARD_COUNT_UNIT, SeqCst);
            return false;
        }
...
mod tests {
    ...
    #[test]
    fn test_full_queue_coalesces_without_corrupting_guard_state() {
        run_guarded_sequencer_test(|sequencer, log| {
            let guard = sequencer.guard();

            for _ in 0..64 {
                assert!(h1!(sequencer, log));
            }
            assert!(!h1!(sequencer, log));
            assert_interrupts_eq!(log, []);
            assert_eq!(64, sequencer.guard_state.load(SeqCst) >> QUEUED_COUNT_SHIFT);

            drop(guard);
            assert_eq!(64, log.borrow().len());
        });
    }
