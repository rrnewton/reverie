...
impl<E: EventSink> Thread<E> {
    ...
    pub fn try_exit(&mut self) -> bool {
        ...
        loop {
            if matches!(self.state, ThreadState::Exiting | ThreadState::Exited) {
                return false;
            }

            if self.compare_and_swap_repr(exiting_state, SeqCst) {
                let _guard = guard::enter_signal_exclusion_zone();
                self.set_state(ThreadState::Exited, Release);
                E::on_thread_exit(self.get_process_and_thread_ids());
                return true;
            }

            // A concurrent exit_all may only have changed needs_to_exit. The
            // failed CAS refreshed self.repr, so retry while this thread still
            // owns an active state.
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_joined_thread_reports_lifecycle_once() {
        run_test_in_new_thread(|| {
            let pid_tid = spawn(|| {
                let mut thread = current_test_thread().expect("Worker thread should be tracked");
                let pid_tid = thread.get_process_and_thread_ids();
                assert!(thread.try_exit());
                assert!(!thread.try_exit());
                pid_tid
            })
            .join()
            ...
            assert!(THREADS_STARTED.lock().unwrap().contains(&pid_tid));
            assert!(THREADS_EXITED.lock().unwrap().contains(&pid_tid));
        });
    }

    #[test]
    fn test_wait_for_all_to_exit_honors_timeout() {
        run_test_in_new_thread(|| {
            let mut thread = current_test_thread().expect("Current thread should be tracked");
            let process_id = thread.get_process_and_thread_ids().pid;
            let started = Instant::now();

            assert!(!wait_for_all_to_exit(
                process_id,
            ...
            assert!(thread.try_exit());
            assert!(wait_for_all_to_exit(process_id, Some(Duration::ZERO)));
        });
    }
}
