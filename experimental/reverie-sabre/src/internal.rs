...
fn guest_task_name(argv0: &[u8]) -> [u8; TASK_COMM_LEN] {
...
unsafe fn set_guest_task_name(argv0: *const libc::c_char) {
    if argv0.is_null() {
        return;
    }

    let name = guest_task_name(unsafe { CStr::from_ptr(argv0) }.to_bytes());
    if name[0] == 0 {
        return;
    }
...
pub fn sbr_init<T: ToolGlobal>(
    ...
    unsafe {
        ...
        *argv = (*argv).wrapping_add(1);
        set_guest_task_name(client_path);
...
#[cfg(test)]
mod task_name_tests {
    use super::guest_task_name;

    #[test]
    fn uses_guest_basename() {
        let name = guest_task_name(b"/usr/bin/bash");
        assert_eq!(&name[..5], b"bash\0");
        assert!(name[5..].iter().all(|byte| *byte == 0));
    }

    #[test]
    fn truncates_to_linux_task_name_limit() {
        let name = guest_task_name(b"/bin/abcdefghijklmnop");
