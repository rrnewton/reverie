...
/// Guest-side view of an inherited SaBRe statistics page.
pub struct SabreGuestStats {
    shared: SharedStats,
}

impl SabreGuestStats {
    /// Maps and validates an inherited SaBRe statistics descriptor.
    ///
    /// # Safety
    ///
    /// The caller must own the descriptor lifecycle for at least as long as
    /// the returned mapping and must ensure it refers to the expected shared
    /// page rather than guest-controlled mutable storage.
    pub unsafe fn from_inherited_fd(fd: RawFd) -> io::Result<Self> {
                ...
                != Some(size_of::<RawBackendStats>())
        {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "invalid SaBRe backend stats descriptor",
            ));
        }
        let shared = SharedStats::map(fd, None)?;
        shared.page().validate()?;
        Ok(Self { shared })
    }

    /// Increments one guest-owned slow-path counter.
    pub fn increment_slow_path(&self, path: SabreSlowPath) {
        self.shared.page().slow_paths[path as usize].fetch_add(1, Ordering::Relaxed);
    }
}
