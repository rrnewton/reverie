...
#[repr(C)]
pub struct DbtRuntimeCallbacks {
...
impl<'a, T> DbtGuest<'a, T>
where
    T: Tool,
{
    /// Creates a DBT guest adapter for an external Reverie tool runtime.
    ...
    // AUTONOMOUS-BOT-IMPLEMENTED
    // TODO-HUMAN-REVIEW(impl-dbi-gap-closure): Review fault-safe DBT memory reads.
...
#[reverie::tool]
impl<T> GlobalRPC<T::GlobalState> for DbtGlobal<'_, T>
where
    T: Tool,
{
    async fn send_rpc(
...
#[reverie::tool]
impl<T> GlobalRPC<T::GlobalState> for DbtGuest<'_, T>
where
    T: Tool,
{
    async fn send_rpc(
        ...
        // AUTONOMOUS-BOT-IMPLEMENTED
        // TODO-HUMAN-REVIEW(#121): Review cross-process GlobalState RPC routing.
        // See `DbtGlobal::send_rpc`: prefer the cross-process coordinator when
...
#[reverie::tool]
impl<T> Guest<T> for DbtGuest<'_, T>
where
    T: Tool,
{
    ...
    // AUTONOMOUS-BOT-IMPLEMENTED
    // TODO-HUMAN-REVIEW(PR-167): Review the DBT guest register-write path.
    ...
    // AUTONOMOUS-BOT-IMPLEMENTED
    // TODO-HUMAN-REVIEW(PR-154): Review exact syscall deferral for DBT tail injection.
    ...
    // AUTONOMOUS-BOT-IMPLEMENTED
    // TODO-HUMAN-REVIEW(PR-ratchet16): Review the DBT in-process guest backtrace.
    fn backtrace(&mut self) -> Option<Backtrace> {
        // The DBT Tool runs *in-process*, in the guest's own address space, so a
        ...
        // frame pointer ends the trace instead of raising SIGSEGV, which in a DBT
...
/// read rather than raising `SIGSEGV`. Used by the DBT [`Guest::backtrace`]
...
/// [`DbtGuest::stack`]. The ptrace backend uses a comparably small window on
/// the tracee's real stack; a page is generous for argument marshalling.
const DBT_STACK_CAPACITY: usize = 4096;
...
impl DbtStack {
    fn new() -> Self {
        Self {
            arena: vec![0u8; DBT_STACK_CAPACITY].into_boxed_slice(),
            offset: 0,
        }
    }
    ...
    fn allocate<'stack, T>(&mut self, value: T) -> AddrMut<'stack, T> {
        ...
        assert!(
            ...
            "DBT guest stack overflow: need {end} bytes, capacity {}",
            self.arena.len()
        );
        ...
        self.offset = end;
        AddrMut::from_raw(ptr as usize).expect("DBT guest stack produced a null scratch pointer")
    }
}

/// Guard that keeps a committed [`DbtStack`] arena alive. Dropping it releases
/// the scratch memory, at which point any addresses handed out become invalid.
pub struct DbtStackGuard {
    _arena: Box<[u8]>,
}

impl Drop for DbtStackGuard {
    fn drop(&mut self) {}
}

impl Stack for DbtStack {
    type StackGuard = DbtStackGuard;
    ...
    fn commit(self) -> Result<Self::StackGuard, Errno> {
        Ok(DbtStackGuard { _arena: self.arena })
    }
}
...
const RNG_SEED_ENV: &str = "HERMIT_DBT_RNG_SEED";
...
fn deterministic_proc_content(path: &Path) -> Option<Vec<u8>> {
    match path.to_str()? {
        ...
        "/proc/self/maps" => {
            Some(b"00400000-00401000 r--p 00000000 00:00 0 [reverie-dbt]\n".to_vec())
        }
        "/proc/self/stat" => {
            let mut contents = String::from("1 (hermit-dbt) R");
            for _ in 0..49 {
                contents.push_str(" 0");
            }
            contents.push('\n');
...
/// Drives an external tool's thread-start lifecycle hook on a DBT guest.
#[allow(clippy::too_many_arguments)]
...
/// Drives an external tool's post-exec lifecycle hook on a DBT guest.
#[allow(clippy::too_many_arguments)]
...
/// Drives one syscall through an external Reverie tool over [`DbtGuest`].
#[allow(clippy::too_many_arguments)]
...
fn run_tool_syscall_inner<T: Tool>(
    ...
    match run_ready(tool.handle_syscall_event(&mut guest, syscall), &tail_result) {
        Some(result) => result.map(DbtSyscallOutcome::Suppress),
...
/// idempotently by each `reverie_dbt_runtime_thread_init` and read with relaxed
...
#[unsafe(no_mangle)]
pub extern "C" fn reverie_dbt_runtime_copied_syscall(_sysnum: i64, _args: *const u64) -> i32 {
    0
}
...
pub unsafe extern "C" fn reverie_dbt_runtime_pre_syscall(
    ...
    let handled = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        if let Some(outcome) = tools::run_active_tool(
            ...
            return match outcome {
                DbtSyscallOutcome::Suppress(value) => {
                    unsafe { result.write(value) };
                    TOTAL_REWRITTEN.fetch_add(1, Ordering::Relaxed);
                    1
                }
...
#[cfg(feature = "prototype-runtime")]
#[unsafe(no_mangle)]
pub extern "C" fn reverie_dbt_runtime_ready(_image_generation: u64) -> i32 {
    1
}

/// Returns the name of the built-in DBT runtime for native summary evidence.
#[cfg(feature = "prototype-runtime")]
#[unsafe(no_mangle)]
...
/// record.
///
/// This is the numeric companion to [`reverie_dbt_runtime_name`]: the native
...
/// [`crate::backend_stats::DbtRuntimeKind::to_wire`]: `Counter1` = 1,
/// `CounterLocal` = 2, `PrototypeTool` = 0.
#[cfg(feature = "prototype-runtime")]
#[unsafe(no_mangle)]
pub extern "C" fn reverie_dbt_runtime_kind_code() -> u8 {
    let kind = if tools::counter1_exact_enabled() {
        crate::backend_stats::DbtRuntimeKind::Counter1
    } else if tools::counter2_exact_enabled() {
        crate::backend_stats::DbtRuntimeKind::CounterLocal
    } else {
        crate::backend_stats::DbtRuntimeKind::PrototypeTool
    };
    kind.to_wire()
}
...
mod tests {
    ...
    #[test]
    fn dbt_stack_allocations_are_valid_in_process_and_aligned() {
        let mut stack = DbtStack::new();
        assert_eq!(stack.size(), 0);
        assert_eq!(stack.capacity(), DBT_STACK_CAPACITY);
    ...
    fn tail_inject_defers_exact_syscall_and_suspends() {
        let mut counters = PrototypeCounters::default();
        let syscall = Syscall::from_raw(Sysno::write, SyscallArgs::new(1, 0x1000, 7, 0, 0, 0));
        let mut guest: DbtGuest<'_, PrototypeTool> = DbtGuest::new(
            0,
            Pid::from_raw(10),
            Pid::from_raw(10),
            None,
    ...
    fn ppid_reflects_constructed_parent_and_root_status() {
        ...
        let root: DbtGuest<'_, PrototypeTool> = DbtGuest::new(
            0,
            Pid::from_raw(10),
            Pid::from_raw(10),
            None,
