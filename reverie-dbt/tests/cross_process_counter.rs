...
//! End-to-end verification of the DBT cross-process GlobalState IPC.
//!
...
//! synchronous [`reverie_dbt::sync_rpc`] client (as the injected DBT guest does)
...
use reverie_dbt::counter::RecordSyscall;
use reverie_dbt::counter::SyscallCounterGlobal;
use reverie_dbt::run_tool_syscall;
use reverie_dbt::sync_rpc;
...
fn record_with_dbt_tool(number: i32) {
    ...
    .expect("DBT counter tool dispatch failed");
    match outcome {
        DbtSyscallOutcome::ExecuteOriginal(original) => {
            assert_eq!(original.number().id(), number)
        }
        DbtSyscallOutcome::Suppress(result) => {
...
fn syscall_counter_global_aggregates_across_fork_tree() {
    ...
    record_with_dbt_tool(PARENT_BEFORE_FORK);
        ...
        match unsafe { libc::fork() } {
            ...
            0 => {
                ...
                for _ in 0..=child_index {
                    record_with_dbt_tool(child_syscall);
                }
                record_with_dbt_tool(SHARED);
    ...
    record_with_dbt_tool(PARENT_AFTER_FORK);
