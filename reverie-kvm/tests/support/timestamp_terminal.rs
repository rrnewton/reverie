// Actual timestamp terminal transitions. These controls exercise the real
    ...
    async fn receive_rpc(&self, from: Pid, (kind, status): Self::Request) {
        ...
        match kind {
            10 => {
                ...
                receiver
                    .await
                    .expect("leader must release the timestamp callback");
            }
            11 => {
                ...
                if let Some(wait) = wait {
                    wait.await
                        .expect("worker must enter its timestamp callback");
                }
                let (sender, receiver) = oneshot::channel();
                {
                    let mut gates = self.gates.lock().unwrap();
...
impl Tool for TerminalTool {
    ...
    async fn handle_rdtsc_event<G: Guest<Self>>(
        ...
        assert_eq!(instruction, Rdtsc::Tsc);
...
fn terminal_program() -> Vec<u8> {
    let mut code = vec![
        0x0f, 0x31, // rdtsc: callback must never return a value to this frame
        0x0f, 0x31, // a resumed frame would produce a forbidden second callback
...
fn run_terminal(mode: u8, code: &[u8]) -> (reverie_kvm::ToolRunCompletion<TerminalLog>, [u8; 16]) {
    ...
    backend
        .install_static_elf(&static_elf(code), "/bin/timestamp-terminal")
        .unwrap();
...
fn terminal_exit_preserves_status_and_consuming_cleanup() {
    if !kvm_available("timestamp terminal exit control") {
        return;
    }
...
fn nonterminal_tails_refuse_before_side_effects() {
    if !kvm_available("timestamp nonterminal tail refusal") {
        return;
    }
    for mode in [3, 4, 5, 6] {
        let (completion, marker) = run_terminal(mode, &terminal_program());
        ...
        let error = completion.result.unwrap_err();
        eprintln!("actual refused timestamp tail mode={mode}: {error:?}");
        let Error::SharedFailure(cause) = error else {
        ...
        assert_eq!(
            *events,
            ...
            "mode={mode}: no new child/image or resumed timestamp",
        );
    }
}
