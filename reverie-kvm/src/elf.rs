...
fn load_executable(
    ...
    if program_headers_address == PROGRAM_HEADERS_ADDRESS {
        memory.map_user_range_with_protection(
...
fn load_segments(
        ...
        .filter(|header| header.p_type == PT_LOAD)
    {
        ...
        if header.p_filesz != 0 {
            let file_mapped_end = align_up(zero_start, PAGE_SIZE)?;
            memory.map_user_range_with_protection(
                mapped_start,
                file_mapped_end - mapped_start,
                protection,
            )?;
        }
        if header.p_memsz > header.p_filesz {
            ...
            let anonymous_length = mapped_end - anonymous_start;
            memory.zero_raw(
                anonymous_start,
                usize::try_from(anonymous_length)
                    .map_err(|_| Error::UnsupportedElf("PT_LOAD BSS is too large".to_string()))?,
            )?;
            memory.map_user_range_with_protection(
                anonymous_start,
                anonymous_length,
                libc::PROT_READ | libc::PROT_WRITE | (protection & libc::PROT_EXEC),
            )?;
        }
...
mod tests {
    ...
    fn fionread_contract_elf_query(
        memory: &GuestMemory,
        mut loaded: LoadedStaticElf,
        output: u64,
    ) -> i64 {
        use crate::SyscallExecutor;
        ...
        let path =
            std::env::temp_dir().join(format!("fionread-elf-{}-{sequence}", std::process::id()));
        std::fs::write(&path, b"abcdefg").unwrap();
        loaded.files.insert(9, std::fs::File::open(&path).unwrap());
        std::fs::remove_file(&path).unwrap();
        let mut executor = crate::executor::ElfExecutor::new(loaded, false);
        let request = crate::SyscallRequest::new(
            libc::SYS_ioctl as u64,
            [9, libc::FIONREAD, output, 0, 0, 0],
        );
        executor.execute(&request, memory)
    }

    #[test]
    fn fionread_contract_elf_initial_permissions_and_privileged_access() {
        for writable in [false, true] {
            let mut image = test_static_elf(&[0x0f, 0x0b]);
            image[68..72].copy_from_slice(&(if writable { 7_u32 } else { 5_u32 }).to_le_bytes());
            ...
            let address = TEST_LOAD_ADDRESS + 0x100;
            memory.write(address, &[0x5a; 8]).unwrap();
            let result = fionread_contract_elf_query(&memory, loaded, address);
            assert_eq!(
                result,
                if writable {
                    0
    ...
    #[test]
    fn fionread_contract_elf_last_load_controls_shared_page() {
        for last_writable in [false, true] {
            let mut image = test_static_elf(&[0x0f, 0x0b]);
            image.resize(0x3000, 0x5a);
            image[56..58].copy_from_slice(&3_u16.to_le_bytes());
            let target = TEST_LOAD_ADDRESS + 0x4000;
            for (index, offset, address, size, flags) in [
                (
                    1_usize,
                    0x2000_u64,
                    target,
                    0x1000_u64,
                    if last_writable { 4_u32 } else { 6_u32 },
                ),
                (
                    2,
                    0x2800,
                    target + 0x800,
                    0x800,
                    if last_writable { 6 } else { 4 },
                ),
            ] {
                let header = 64 + index * 56;
                image[header..header + 4].copy_from_slice(&PT_LOAD.to_le_bytes());
                image[header + 4..header + 8].copy_from_slice(&flags.to_le_bytes());
                for (field, value) in [
                    (8, offset),
                    (16, address),
                    (24, address),
                    ...
                    image[header + field..header + field + 8].copy_from_slice(&value.to_le_bytes());
                }
            }
            let mut memory = GuestMemory::new(0, TEST_MEMORY_SIZE).unwrap();
            let loaded = load_static_elf(
            ...
            memory.enable_user_access();
            assert_eq!(
                fionread_contract_elf_query(&memory, loaded, target + 0x100),
                if last_writable {
                    0
                } else {
                    -i64::from(libc::EFAULT)
    ...
    #[test]
    fn fionread_contract_elf_bss_mapping_permissions_and_zeroes() {
        for (offset, file_size, memory_size) in
            [(0_u64, 0_u64, 4096_u64), (2048, 0, 2048), (0, 2048, 8192)]
        {
            for last_writable in [false, true] {
                for output_offset in [256_u64, 4352] {
                    if memory_size < 8192 && output_offset >= 4096 {
                        continue;
                    }
                    let mut image = test_static_elf(&[0x0f, 0x0b]);
                    image.resize(0x4000, 0x5a);
                    image[56..58].copy_from_slice(&3_u16.to_le_bytes());
                    let target = TEST_LOAD_ADDRESS + 0x4000;
                    for (index, address, file_offset, filesz, memsz, flags) in [
                        (
                            1_usize,
                            target,
                            0x2000_u64,
                        ...
                        for (field, value) in [
                            ...
                            image[header + field..header + field + 8]
                    ...
                    .unwrap();
                    memory.enable_user_access();
                    let anonymous_start = if file_size == 0 {
                        target
                    } else {
                        target + 4096
                    };
                    let mut anonymous = vec![0x5a; 4096];
                    memory.read(anonymous_start, &mut anonymous).unwrap();
                    assert_eq!(
                        anonymous,
                        vec![0; 4096],
                        "offset={offset} filesz={file_size} writable={last_writable}"
                    );
                    let mut before = vec![0; 8192];
                    memory.read(target, &mut before).unwrap();
                    let allowed = file_size == 0 || output_offset >= 4096 || last_writable;
                    assert_eq!(
                        fionread_contract_elf_query(&memory, loaded, target + output_offset),
                        if allowed { 0 } else { -i64::from(libc::EFAULT) },
                        "offset={offset} filesz={file_size} writable={last_writable} output={output_offset}"
                    );
                    let mut expected = before;
                    if allowed {
                        expected[output_offset as usize..output_offset as usize + 4]
                            .copy_from_slice(&7_i32.to_ne_bytes());
                    }
                    let mut after = vec![0; 8192];
                    memory.read(target, &mut after).unwrap();
    ...
    #[test]
    fn fionread_contract_elf_reload_resets_permissions_and_keeps_stack_writable() {
        let mut memory = GuestMemory::new(0, TEST_MEMORY_SIZE).unwrap();
        for writable in [false, true, false] {
            ...
            .unwrap();
            let stack_output = loaded.stack_pointer - 0x100;
            memory.enable_user_access();
            assert_eq!(
                fionread_contract_elf_query(&memory, loaded, stack_output),
                0
            );
            let loaded = load_static_elf(
                &mut memory,
            ...
            memory.enable_user_access();
            assert_eq!(
                fionread_contract_elf_query(&memory, loaded, TEST_LOAD_ADDRESS + 0x100),
                if writable {
                    0
                } else {
                    -i64::from(libc::EFAULT)
