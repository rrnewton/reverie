...
// TODO-HUMAN-REVIEW(PR-156): Root process syscalls (fork/vfork/clone/execve/wait4)
// are intentionally NOT backend-owned so they dispatch through the Reverie Tool;
// only worker-coordination syscalls stay in the KVM personality.
...
// TODO-HUMAN-REVIEW(PR-156): Review root process-syscall Tool injection completion.
...
impl GuestSyscallExecutor for StaticElfSyscallExecutor<'_> {
    ...
    fn complete_injection(&mut self) -> Result<InjectionCompletion> {
        let Some(action) = self.executor.take_process_action() else {
            // TODO-HUMAN-REVIEW(PR-156): PR #156's base returned
            // `DoesNotReturn { process_exited: true }` here whenever the injected
            // syscall left a pending exit (exit/exit_group). On current `main`,
            // `execute()` sets `exit_code` for exit/exit_group *before*
            // `complete_injection` runs, and the `run_static_elf_with_tool` loop
            // now drains that pending exit through `take_exit()` ->
            // `request_guest_thread_group_exit`/`cancel_guest_threads` *after* the
            // handler future returns. Making `inject` non-returning for a plain
            // exit would (a) strand any tool that does post-`inject` work — e.g.
            // `StraceTool` records via `send_rpc` only after `inject` returns — and
            // (b) contradict main's ratcheted ptrace-fidelity expectation that
            // `exit_group` is observed. A plain exit therefore returns normally;
            // only image replacement (below) genuinely cannot resume the handler.
            return Ok(InjectionCompletion::Returns);
        };
...
impl KvmBackend {
        ...
        T: Tool,
    {
        ...
        let handler_signal = Arc::new(Mutex::new(None));
        expose_tool_scratch(&memory)?;
        // TODO-HUMAN-REVIEW(PR-156): Thread-start injections can now complete a
        // root process action; the lifecycle executor tracks that completion.
        let mut _process_completed = false;
        let start_outcome = {
            let mut guest_executor = StaticElfSyscallExecutor {
                backend: self,
        ...
        // the same successful-exec lifecycle boundary as ptrace before running
        // it. A tool that re-execs from `handle_post_exec` is re-delivered the
        // lifecycle boundary by `run_post_exec_handler`.
        ...
        loop {
            ...
            let request = SyscallRequest::read_from(&memory, frame_address)?;
            let syscall = request.into_syscall()?;
            // TODO-HUMAN-REVIEW(PR-156): Root process-syscall Tool dispatch composed
            // with the KVM worker-owned syscall gating from the #173 ratchet. Process
            // syscalls (fork/clone/exec/wait4) are not backend-owned, so a subscribed
            // tool receives them and can defer their completion through injection.
            ...
            // the vCPU is stopped and not re-run while the tool callback is active.
            unsafe {
                (return_slot as *mut u64).write(0);
            }
            // A process action completed inside the handler (fork/clone) already
            // wrote its result into the frame; re-establish the resumed-syscall
            // return so the guest observes the injected value.
            if handler_process_completed && !handler_replaced_image {
                configure_process_syscall_return(&memory, &self.vcpu, frame_address, result, None)?;
            }
