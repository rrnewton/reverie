...
fn clear_tid_before_tool_exit(memory: &mut GuestMemory, executor: &mut ElfExecutor) {
    // Consume the address so the worker wrapper cannot overwrite memory after
    // the callback observes Linux's pre-notification zero and wake.
...
impl KvmBackend {
        ...
        <T::GlobalState as GlobalTool>::Config: 'static,
    {
        ...
        expose_tool_scratch(memory)?;
        let boundary = CompletedSyscallBoundary::capture(self, frame_address, Some(registers))?;
        let outcome = {
            let mut guest_executor = StaticElfSyscallExecutor {
                backend: self,
                executor,
                memory: memory.clone(),
                process_context: ProcessExecutionContext::SignalBoundary(boundary),
                last_result: None,
        ...
        <T::GlobalState as GlobalTool>::Config: 'static,
    {
        ...
        if self.guest_thread_is_cancelled() {
            ...
            notify_tool_exit(
                ...
                config,
                thread_state,
                ToolExit::for_executor(executor, ExitStatus::SUCCESS),
            )
            .await?;
