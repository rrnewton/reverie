...
pub(crate) fn with_dispatch_mask<Result>(
    ...
    let guest_mask = unsafe { &mut *(guest_mask as *mut libc::sigset_t).cast::<u64>() };
    unsafe { with_ordinary_dispatch_mask(guest_mask, signal::runtime_handler_mask(), dispatch) }
}

/// Run an ordinary deferred dispatch with a live, scalar kernel mask.
///
/// # Safety
/// The caller owns the mask and matching runtime exclusion until this returns.
/// The mask must satisfy the backend's signal policy. No frame reference may
/// survive a sigreturn; only its copied kernel mask may be passed here.
pub unsafe fn with_ordinary_dispatch_mask<Result>(
    guest_mask: &mut u64,
    runtime_mask: u64,
    dispatch: impl FnOnce() -> Result,
) -> Result {
...
/// Copy the current activation's kernel mask, without retaining its frame.
pub fn dispatch_mask() -> Option<u64> {
    let scope = DISPATCH_SCOPE.get();
    (!scope.is_null()).then(|| unsafe { *(*scope).guest_mask })
}

/// Forward a guest syscall under the current activation's captured mask.
///
/// # Safety
/// Apply backend syscall/descriptor guards first. Arguments must obey the
/// syscall's memory requirements. Internal runtime controls must instead use
/// the raw gate. A nested signal activation must own a separate mask scope.
pub unsafe fn forward_syscall(number: i64, args: [u64; 6]) -> i64 {
    let scope = DISPATCH_SCOPE.get();
    if scope.is_null() {
    ...
    let closed = unsafe {
        trap::raw_syscall6(
            ...
            [
                libc::SIG_SETMASK as u64,
                (&raw const scope.runtime_mask) as u64,
    ...
    unsafe { scope.guest_mask.write(guest_mask) };
    ...
    unsafe fn install(&self, config: &RuntimeConfig) -> io::Result<()> {
        unsafe {
            ...
            if signal::runtime_signals_configured() {
                let mask = signal::runtime_ordinary_mask();
                syscall_result(trap::raw_syscall6(
                    libc::SYS_rt_sigprocmask,
                    [
                        libc::SIG_SETMASK as u64,
