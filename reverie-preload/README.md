...
An ordinary deferred backend may instead copy the eight-byte kernel mask and
establish `with_ordinary_dispatch_mask` around its typed Tool dispatch. Its actual
guest injection paths must use scoped forwarding, not the internal raw gate.
`defer_to_clocked_with_mask` changes the original frame's return mask independently
of the two-word clock continuation. The backend must restore the copied guest mask
at its trusted assembly tail, after all Rust cleanup and before clock enable.

`signal::configure_runtime_signals` is an unsafe, startup-only registration for
explicit runtime SIGTRAP/SIGUSR2 sources, not general guest signal virtualization.
Sources remain disabled during installation. Each source supplies a validator and
scalar-only body; the installed wrapper stops clock accounting before validation,
rejects unknown sources, and optionally disables its owned breakpoint before
entering clock-control code. Required blocked/ignored signals are refused, never
silently unblocked. Callers must preserve the descriptor/code/state lifetime,
exclude ordinary handler reentry and terminate on post-publication setup failure.
LiteInst implements those stricter admission and syscall guards for its bounded
zero-patch clock path. This registration does not qualify timer delivery.
