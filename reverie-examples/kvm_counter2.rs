...
//! Runs the exact shared counter2 tool through the Reverie KVM guest.
...
async fn run() -> Result<i32, Box<dyn Error>> {
    ...
    let executable = PathBuf::from(
        args.next()
            .ok_or("usage: reverie-kvm-counter2 PROGRAM [ARG ...]")?,
    );
    ...
    let argv: Vec<String> = std::iter::once(executable.as_os_str().to_owned())
        ...
        .map(|value| {
            value
                .into_string()
                .map_err(|_| "KVM counter2 arguments must be valid UTF-8")
        })
        .collect::<Result<_, _>>()?;
    ...
    let (global, status, _, _) = backend
        ...
        .await?;
    let (total, processes, threads) = global.totals();
    eprintln!(
...
fn main() {
    let status = match tokio::runtime::Builder::new_current_thread()
        ...
        Ok(status) => status,
        Err(error) => {
            eprintln!("reverie-kvm-counter2: {error}");
            1
        }
    };
