...
# The non-ptrace backends and their shared transport are first-class public
# targets too. Keep these declarations next to the older exported targets so a
# standalone Reverie checkout and Hermit's `reverie` cell see the same graph.
rust_library(
...
rust_library(
    ...
    autocargo = {"cargo_toml_dir": "reverie-kvm"},
    deps = [
        "fbsource//third-party/rust:futures",
        "fbsource//third-party/rust:goblin",
...
# LiteInst embeds the preload runtime with coordinator RPC enabled. Building
# this single target therefore proves the in-process backend, preload runtime,
# ptrace fallback, and RPC transport all share one Buck dependency graph.
rust_library(
    name = "reverie-preload",
    srcs = glob(["reverie-preload/src/**/*.rs"]),
    autocargo = {"cargo_toml_dir": "reverie-preload"},
...
rust_library(
    name = "reverie-liteinst",
    srcs = glob(["reverie-liteinst/src/**/*.rs"]),
    autocargo = {"cargo_toml_dir": "reverie-liteinst"},
    features = ["preload-constructor"],
    deps = [
        "fbsource//third-party/rust:bincode",
        "fbsource//third-party/rust:libc",
