...
unsafe fn inherit_fd_for_exec(fd: i32) -> Result<(), Errno> {
    unsafe {
        let flags = Errno::result(libc::fcntl(fd, libc::F_GETFD)).inspect_err(|_| {
            crate::report_pre_exec_failure(
                libc::STDERR_FILENO,
                b"reverie pre_exec inherit_fd: fcntl F_GETFD failed\n",
            );
        })?;
        Errno::result(libc::fcntl(fd, libc::F_SETFD, flags & !libc::FD_CLOEXEC)).inspect_err(
            |_| {
                crate::report_pre_exec_failure(
                    libc::STDERR_FILENO,
                    b"reverie pre_exec inherit_fd: fcntl F_SETFD failed\n",
                );
            },
        )?;
        Ok(())
...
mod tests {
    ...
    fn pre_exec_inherit_fd_diagnostic_preserves_error() {
        ...
        for closed_stderr in [false, true] {
            ...
            assert_eq!(
                error.to_string(),
                io::Error::from_raw_os_error(libc::EBADF).to_string()
            );
            use std::io::Seek;
            ...
            assert_eq!(
                ...
                if closed_stderr {
                    ...
                    "reverie pre_exec inherit_fd: fcntl F_GETFD failed\n"
                }
            );
        }
    }
