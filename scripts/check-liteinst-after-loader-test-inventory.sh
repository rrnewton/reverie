...
if ! metadata=$(CARGO_TERM_COLOR=never cargo metadata --no-deps --format-version=1); then
...
readonly -a after_loader_tests=(
    exact_clock_trajectory_comparator_refuses_mutation_loss_duplication_and_reordering
    ...
    installed_hook_timer_transport_deopts_to_exact_ptrace_trajectory
