...
pub(crate) fn waitpidfd_raw(raw_fd: RawFd, flags: WaitPidFlag) -> Result<WaitPidfdRaw, Errno> {
    ...
    #[cfg(test)]
    waitid_si(IdType::Pidfd(raw_fd), flags)
        .map(|raw| WaitPidfdRaw(WaitPidfdRawInner::Kernel(raw)))
}
...
fn valid_but_typed_unsupported_siginfo(code: i32, si_status: i32) -> bool {
    match code {
        libc::CLD_KILLED | libc::CLD_DUMPED | libc::CLD_STOPPED => {
            valid_linux_signal_number(si_status)
                && Signal::try_from(si_status).is_err()
        }
        libc::CLD_TRAPPED => {
            ...
            event == 0
                && signal != libc::SIGKILL
                && valid_linux_signal_number(signal)
...
mod tests {
    ...
    fn exact_wait_oracle_fails_closed_on_valid_unsupported_realtime_signals() {
        ...
        for code in [
            ...
            assert_eq!(
                classify_physical_wait_siginfo(
                    libc::SIGCHLD,
                    0,
                    code,
                    pid,
                ...
                Ok(PhysicalWaitSiginfoClass::ValidButTypedUnsupported),
                "lost valid-but-typed-unsupported RT status"
            );
            assert_eq!(
