//! Aggregate statistics for LiteInst patch-site test coverage.

use std::fmt;

use crate::InstructionPatchShape;
use crate::PatchShapeCollector;
use crate::PatchShapeStats;
...
/// Installation outcome for a distinct LiteInst patch candidate.
pub enum LiteinstPatchOutcome {
    /// Reserved for the planner's RapidPreferred path.
    #[allow(dead_code)]
    DirectPunPatched,
    /// Installed through relocation.
    RelocatedPatched,
    /// The patch was rejected because it straddles a cache line.
    PtraceStraddlerBail,
    /// The patch was rejected for another installation limitation.
    PtraceOtherFallback,
}
...
impl LiteinstInstrumentationStats {
    ...
    /// Records a candidate once per process and exec generation.
    ...
    /// Records a first-site seccomp dispatch.
    ...
    /// Returns first-site observation, successful installation, straddler fallback,
    /// other fallback, and installed-hook dispatch counts.
