...
//!
//! This type lives in the SHARED crate, not in `reverie-ptrace`, and that placement is
//! load-bearing rather than cosmetic. These counters describe what the LiteInst *patching engine*
//! did -- which sites were patched, which fell back, which dispatch path each syscall took. They
//! are not a property of the ptracer. While they were owned by `reverie-ptrace` only a run with a
//! ptrace host could populate them, so the in-guest path had no way to report the same numbers,
//! and flipping LiteInst in-guest would have silently zeroed them. A counter that reads zero
//! because nothing recorded it is indistinguishable from a counter that reads zero because the
//! fallback never fired -- which is exactly the signal this gate depends on.
//!
//! `PatchShapeStats` and `PatchShapeCollector`, which these counters are built from, already live
//! here; this puts the aggregate next to its parts.

use std::fmt;
use std::sync::Arc;
use std::sync::Mutex;

use crate::backend_stats::InstructionPatchShape;
use crate::backend_stats::PatchShapeCollector;
use crate::backend_stats::PatchShapeStats;
...
/// What a LiteInst patch attempt did to one site.
///
/// Recorded per site so a run can distinguish "patched on the fast path" from "fell back", which
/// is the difference between the in-guest path working and silently degrading.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum LiteinstPatchOutcome {
    /// Reserved for the planner's RapidPreferred path; the current host uses
    /// replace-first relocation for syscall instructions.
    #[allow(dead_code)]
    DirectPunPatched,
    /// The original instruction was relocated and the site patched. The normal success path for
    /// syscall instructions.
    RelocatedPatched,
    /// Declined: the original instruction's first five bytes cross a cache line, so a direct-jump
    /// patch could not be installed atomically.
    PtraceStraddlerBail,
    /// Declined for any other reason; the site keeps its trap-based dispatch.
    PtraceOtherFallback,
}
...
impl LiteinstInstrumentationStatsHandle {
    /// Wraps shared instrumentation state in a cloneable observer.
    pub fn from_shared(inner: Arc<Mutex<LiteinstInstrumentationStats>>) -> Self {
        Self { inner }
    }
...
impl LiteinstInstrumentationStats {
    #[cfg(test)]
    /// Records one candidate patch site and the shape of the instruction found there.
    pub fn record_site(
        &mut self,
        rip: u64,
    ...
    /// Records a site observed for a specific process.
    ...
    /// Records a dispatch that took the first-site seccomp path.
    pub fn record_first_site_seccomp(&mut self) {
        self.dispatch_paths[0] += 1;
    }

    /// Records a dispatch that required a ptrace-hosted installation. This is the B-class path: a nonzero count here means the ptracer is still on the syscall path for this run.
    pub fn record_ptrace_installation(&mut self) {
        self.dispatch_paths[1] += 1;
    }

    /// Records a dispatch that fell back because the site straddles a cache line.
    pub fn record_cacheline_straddler_fallback(&mut self) {
        self.dispatch_paths[2] += 1;
    }

    /// Records a dispatch that fell back for any other reason.
    pub fn record_unpatchable_or_other_fallback(&mut self) {
        self.dispatch_paths[3] += 1;
    }

    /// Records a dispatch that took the in-guest direct hook. This is the fast path.
    pub fn record_direct_hook(&mut self) {
...
/// Runs `record` against locked instrumentation state, or returns `None` when collection is off.
pub fn with_liteinst_stats<R>(
