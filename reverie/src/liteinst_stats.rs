...
/// Host-hybrid installation outcome for a distinct LiteInst patch candidate.
pub enum LiteinstPatchOutcome {
    ...
    RelocatedPatched,
    /// Retained ptrace dispatch because the patch straddles a cache line.
    PtraceStraddlerBail,
    /// Retained ptrace dispatch for another installation limitation.
    PtraceOtherFallback,
}
...
impl LiteinstInstrumentationStats {
    ...
    /// Records a host-hybrid candidate once per process and exec generation.
    ...
    /// Records a successful ptrace installation.
