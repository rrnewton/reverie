...
use crate::Errno;
use crate::pmu::config::get_pmu_config;
use crate::pmu::config::has_precise_ip;

#[cfg(test)]
mod logging_tests;
use crate::pmu::perf::PerfCounter;
use crate::pmu::perf::do_branches;
...
impl Drop for ScopedFd {
    fn drop(&mut self) {
        if let Err(errno) = Errno::result(unsafe { libc::close(self.0) }) {
            warn!(target: "reverie_ptrace::validation", "Error while closing file descriptor - {:?}", errno);
        }
    }
}
    ...
    match &fd_result {
        Err(Errno::EINVAL) if attr.config & IN_TXCP > 0 => {
            ...
            if no_txcp_fd.is_ok() {
                ...
                warn!(target: "reverie_ptrace::validation", "kernel does not support IN_TXCP");
            }
...
mod test {
    use super::*;
    use crate::pmu::perf::is_perf_supported;
