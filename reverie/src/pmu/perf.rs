...
use crate::Errno;
use crate::Tid;
use crate::pmu::validation::PmuValidationError;
use crate::pmu::validation::check_for_pmu_bugs;
...
impl Builder {
    ...
    pub fn check_for_pmu_bugs(&mut self) -> &mut Self {
...
impl PerfCounter {
    ...
    #[inline(always)]
    pub(crate) fn sample_rdpmc_once(&self) -> crate::pmu::InGuestRcbSample {
        #[cfg(target_arch = "x86_64")]
        {
        ...
        #[cfg(not(target_arch = "x86_64"))]
        {
            crate::pmu::InGuestRcbSample::Unavailable
        }
    }
    ...
    fn sample_rdpmc_once_using(
    ...
    ) -> crate::pmu::InGuestRcbSample {
        use std::ptr::addr_of;
        use std::ptr::addr_of_mut;
        use std::ptr::read_volatile;

        use crate::pmu::InGuestRcbSample;
        ...
        let sample = if index == 0 || enabled != running {
            ...
            crate::pmu::in_guest_timer::sampled_count(offset, read_pmc(index - 1), width)
        };
        smp_rmb();
        if sequence != unsafe { read_once(addr_of_mut!((*page).lock)) } {
...
fn handle_perf_pmu_error(errno: Errno) -> bool {
    match errno {
        ...
        _ => {
            warn!(target: "reverie_ptrace::perf", "Perf feature check failed unexpectedly due to {errno}; assuming unsupported");
        }
    }
...
macro_rules! ret_without_perf {
    () => {
        if !$crate::pmu::is_perf_supported() {
            return;
        }
    };
    (expr:expr) => {
...
mod test {
    ...
    fn rdpmc_preserves_one_branch_intervals() {
        let config = crate::pmu::PmuConfig::try_new().expect("hardware RCB PMU required");
