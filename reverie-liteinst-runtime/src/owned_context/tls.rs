...
unsafe extern "C" {
    #[linkage = "extern_weak"]
    static pl_tls: *const Control;
    #[linkage = "extern_weak"]
    static pl_guest_arch_prctl: Option<unsafe extern "C" fn(u64, u64, *mut Observation)>;
}
...
pub(crate) fn available() -> bool {
    unsafe { !pl_tls.is_null() && pl_guest_arch_prctl.is_some() }
}

fn control() -> Result<&'static Control, &'static str> {
    if !available() {
        return Err("private CRT provider unavailable");
    }
    unsafe { pl_tls.as_ref() }.ok_or("private CRT provider unavailable")
}

fn state() -> Result<State, &'static str> {
    control().map(snapshot)
}

pub(crate) fn installed() -> bool {
    unsafe { pl_tls.as_ref() }.is_some_and(|control| control.phase.load(Ordering::Acquire) != 0)
}
...
pub(crate) fn inject(args: [u64; 6]) -> Result<i64, &'static str> {
    ...
    let arch_prctl = unsafe { pl_guest_arch_prctl }.ok_or("private CRT provider unavailable")?;
    let execute = || {
        execute_with(
            before,
            kernel_tid(),
            ...
            || {
                ...
                unsafe { arch_prctl(args[0], args[1], &mut observed) };
                observed
            },
        )
    };
    ...
    publish(control, before, args[0], args[1], observed)
}
