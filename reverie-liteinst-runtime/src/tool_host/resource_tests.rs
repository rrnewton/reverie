...
impl Tool for TerminalTool {
    ...
    async fn on_exit_process<G: GlobalRPC<()>>(
        ...
        assert_eq!(*self.events.lock().unwrap(), ["new", "thread", "state-drop"]);
        assert_eq!(self.resources.as_ref().unwrap().active(), 2);
...
fn terminal_path(fail_process: bool) {
    ...
    let mut invocation = registry.reserve(pid, tid, rpc.clone(), &scratch.owner, true).unwrap();
    invocation.initialize(None).unwrap();
    let result = finish_tool_exit_callbacks(
        &registry,
        invocation,
        ToolExitContext { tid, pid, number: libc::SYS_exit_group, args: [7, 0, 0, 0, 0, 0] },
        |_| {
    ...
    if fail_process {
        ...
        assert_eq!(*events.lock().unwrap(), ["new", "thread", "state-drop", "process"]);
        assert_eq!(resources.active(), 1);
        assert!(rpc.raw_fd() >= 0);
        rpc.retire().unwrap();
        ...
        assert_eq!(*events.lock().unwrap(), ["new", "thread", "state-drop", "process", "stats", "retire"]);
...
fn callback_drain_precedes_terminal_rpc_and_final_resource_drain() {
    isolated("tool_host::resource_tests::callback_drain_precedes_terminal_rpc_and_final_resource_drain", || terminal_path(false));
}

#[test]
fn failed_terminal_rpc_hook_does_not_claim_resource_drain() {
    isolated("tool_host::resource_tests::failed_terminal_rpc_hook_does_not_claim_resource_drain", || terminal_path(true));
}
