...
> {
    ...
    for (register, bytes) in registers
        .iter_mut()
        .zip(image.frame_bytes()[48..232].chunks_exact(8))
    {
        *register = i64::from_le_bytes(bytes.try_into().map_err(|_| Error::Metadata)?);
    }
...
mod tests {
    ...
    fn terminal126_owned_direct_and_closure_sites_survive_ordinary_child_exit() {
        ...
        fn closure_site(active: bool) {
            let error = crate::mapping::Failure::ordinary("host-mapping-control", Some(-22));
            if active {
                Err::<(), _>(error).unwrap_or_else(|error| super::refuse_error(error));
            }
        }
        ...
        for opcode in [&[0x48, 0x8b, 0x04, 0xc7][..], &[0xff, 0x24, 0xc7]] {
            for kind in [Kind::Cpuid, Kind::Rdtsc, Kind::Rdtscp] {
                ...
                let mut state = OwnedContext {
                    owner: 1,
                    mappings: vec![pc..pc + code.len() as u64],
                    readable: vec![address..address + core::mem::size_of_val(&data) as u64],
                    writable: Vec::new(),
                    vdso: None,
                    signal_stack: 0..0,
                    runtime_sp: 0,
...
mod initial_tests {
    ...
    fn step_state(code: &[u8]) -> OwnedContext {
        ...
        state.mappings = vec![pc..pc + code.len() as u64];
        state
    }
    ...
    mod rng_connected {
        ...
        fn descriptor() -> Box<RetainedImage> {
            ...
            let bytes =
                std::fs::read(std::env::var_os("LITEINST_RNG_DESCRIPTOR_ELF").unwrap()).unwrap();
