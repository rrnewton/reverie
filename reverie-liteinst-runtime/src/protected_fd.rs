...
fn sendmsg_validation_args(args: [u64; 6]) -> [u64; 6] {
    [args[0], usize::MAX as u64, args[2], 0, 0, 0]
}

fn inspect(number: i64, args: [u64; 6], protected: &[i32]) -> Result<(), i32> {
    match number {
        libc::SYS_sendmsg => message(args[1], protected).map_err(|error| {
            let validation =
                unsafe { raw_syscall6(libc::SYS_sendmsg, sendmsg_validation_args(args)) };
            if validation < 0 && validation != -i64::from(libc::EFAULT) {
                (-validation) as i32
            } else {
                error
            }
