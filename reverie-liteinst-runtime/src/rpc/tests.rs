...
thread_local! {
    static PUBLISHED: RefCell<Option<Box<dyn FnOnce(i32)>>> = RefCell::new(None);
    static BEFORE_CLOSE: RefCell<Option<Box<dyn FnOnce(i32)>>> = RefCell::new(None);
}
...
pub(crate) fn isolated(name: &str, body: impl FnOnce()) {
    ...
    let status = std::process::Command::new(std::env::current_exe().unwrap())
        ...
        .status()
        .unwrap();
    assert!(status.success(), "isolated resource control failed: {status}");
}
...
fn fd_open(fd: i32) -> bool {
    unsafe { raw_syscall6(libc::SYS_fcntl, [fd as u64, libc::F_GETFD as u64, 0, 0, 0, 0]) >= 0 }
}
...
fn bootstrap_publication_precedes_delayed_config_without_callback() {
    isolated("rpc::tests::bootstrap_publication_precedes_delayed_config_without_callback", || {
        ...
        let server = std::thread::spawn(move || {
            ...
            write_frame(&mut peer, &reverie_rpc_transport::codec::encode(&()).unwrap());
            let mut byte = [0];
            assert_eq!(peer.read(&mut byte).unwrap(), 0);
        });
        let client = std::thread::spawn(move || {
            PUBLISHED.with(|slot| *slot.borrow_mut() = Some(Box::new(move |fd| published_tx.send(fd).unwrap())));
...
fn bootstrap_connect_error_retires_published_resource() {
    isolated("rpc::tests::bootstrap_connect_error_retires_published_resource", || {
        let directory = tempfile::tempdir().unwrap();
        let resources = resources::Resources::new();
        let recorded = Arc::new(AtomicI32::new(-1));
        let capture = recorded.clone();
        PUBLISHED.with(|slot| *slot.borrow_mut() = Some(Box::new(move |fd| capture.store(fd, Ordering::Release))));
        let error = CoordinatorRpc::<()>::connect_bootstrap(&directory.path().join("missing"), resources.clone()).err().unwrap();
...
fn bootstrap_config_error_and_publication_unwind_release_exact_owner() {
    isolated("rpc::tests::bootstrap_config_error_and_publication_unwind_release_exact_owner", || {
        for unwind in [false, true] {
            ...
            let recorded = Arc::new(AtomicI32::new(-1));
            let capture = recorded.clone();
            PUBLISHED.with(|slot| *slot.borrow_mut() = Some(Box::new(move |fd| {
                capture.store(fd, Ordering::Release);
                assert!(!unwind, "publication unwind control");
            })));
            let server = (!unwind).then(|| std::thread::spawn(move || {
                let (peer, _) = listener.accept().unwrap();
                drop(peer);
            }));
            let result = catch_unwind(AssertUnwindSafe(|| CoordinatorRpc::<()>::connect_bootstrap(&path, resources.clone())));
            if unwind {
...
fn bootstrap_conflicting_slot_preserves_existing_owner() {
    isolated("rpc::tests::bootstrap_conflicting_slot_preserves_existing_owner", || {
        let directory = tempfile::tempdir().unwrap();
...
fn native_close_stays_counted_and_published_through_blocking_destructor() {
    isolated("rpc::tests::native_close_stays_counted_and_published_through_blocking_destructor", || {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("rpc");
        let listener = UnixListener::bind(&path).unwrap();
        let server = std::thread::spawn(move || {
        ...
        let worker = std::thread::spawn(move || {
            BEFORE_CLOSE.with(|slot| *slot.borrow_mut() = Some(Box::new(move |actual_fd| {
        ...
        release.wait();
        worker.join().unwrap();
        assert!(matches!(drain.as_mut().poll(&mut context), Poll::Ready(Ok(()))));
        assert!(!fd_open(fd) && !protected(fd));
        server.join().unwrap();
    });
}
...
fn failed_resource_cleanup_is_not_successful_drain() {
    ...
    resources.close();
    drop(pending);
    assert_eq!(reverie_preload::tool_host::drive_ready(resources.drain()).unwrap_err().raw_os_error(), Some(libc::EIO));
}
