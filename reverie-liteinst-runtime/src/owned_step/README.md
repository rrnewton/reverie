...
are not credited as retired TF steps. Original zero/default XSAVE comparisons
remain live failures; a seeded profile is additional evidence only.
...
Build the test with `--no-run`, then build its paired guest using `cargo rustc`
with `-C opt-level=2` in the same private target. The test embeds that exact guest
path; freeze both hashes after the final build. Runtime preflight must reject
exported `LD_*`, `GLIBC_TUNABLES` and any `/etc/ld.so.preload`. Invoke frozen tests
directly (Cargo may add loader variables) only after independent review and
resource admission, with a short external `REVERIE_COMPILED_STEP_ARTIFACTS` path.
