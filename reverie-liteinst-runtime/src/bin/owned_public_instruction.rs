...
//! `runtime::initialize_from_environment`. That function refuses the removed
//! `REVERIE_LITEINST_TOOL` launch selector and **otherwise returns `Ok(())`
//! without installing any runtime, signal, seccomp or SUD state**.
//!
...
//! backstop only — the constructor has already run by the time `main` begins.
//!
...
//! # Child environment
//!
//! `cargo test` injects `LD_LIBRARY_PATH` into a test process. The harness preserves
//! the parent environment and removes loader-only variables from each server and
//! guest child command. Product constructor/runtime selectors remain refused at
//! each spawn boundary.
//!
