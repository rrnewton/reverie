...
fn active() -> Active {
    ...
    Active {
        ...
        vvar: vec![0xf000..0x10000],
        exports: elf::exports(&bytes, 0x10000..0x11000).unwrap(),
        writable: vec![0x20000..0x21000],
        changes: Vec::new(),
        _image: bytes,
    }
}
...
fn data_fault_diagnostic_interval_edges_high_bits_and_no_match() {
    ...
    owner.vvar = vec![0x1234_0000_e000..0x1234_0001_0000];
...
fn f1_recognized_call_returns_to_owned_text_armed_and_unarmed() {
    ...
    for ticket in [
        ...
        for target in [0x10010, 0x10800] {
            ...
            assert!(owner.call(fault(), target, &[0x30000..0x31000]).is_none());
