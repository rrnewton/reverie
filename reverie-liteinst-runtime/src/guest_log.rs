...
mod buffered;
pub(crate) use buffered::complete_fork;
pub(crate) use buffered::prepare_fork;
...
pub struct GuestLog(OwnedFd, LogVersion);
...
impl GuestLog {
    ...
    /// Buffered mode requires the existing clocked constructor/runtime boundary
    /// whenever the guest counter is requested or active. All writers retain the
    /// protected lifetime endpoint until process exit, including after FINISH;
    /// forks use the existing COW handoff and successful exec remains unsupported.
    pub unsafe fn install(self) -> io::Result<GuestLogWriter> {
        let buffered = match self.1 {
            LogVersion::Buffered => Some(buffered::attach(self.0.as_raw_fd())?),
            LogVersion::Ordered => Some(buffered::attach_ordered(self.0.as_raw_fd())?),
            LogVersion::Packets => None,
        };
        LOG_FD
        ...
        if let Some(buffered) = buffered {
            unsafe { buffered::install(buffered) };
        }
        start_process();
        Ok(GuestLogWriter(()))
    }
...
impl GuestLogWriter {
    pub fn record_failed(&mut self) -> io::Result<()> {
        if !buffered::ordered() {
            return Err(io::Error::other(
                "record failure reporting requires prepared capture",
            ));
        }
        buffered::mark_failed();
        Ok(())
    }
    pub fn write_record(
        &mut self,
        bytes: &[u8],
    ) -> io::Result<reverie_rpc_transport::guest_log::RecordCommit> {
        buffered::write_ordered(bytes)
    }
}
...
impl Write for GuestLogWriter {
    fn write(&mut self, bytes: &[u8]) -> io::Result<usize> {
        if buffered::ordered() {
            buffered::mark_failed();
            return Err(io::Error::other(
                "ordered capture requires explicit complete-record emission",
            ));
        }
        if buffered::write(bytes) {
            return Ok(bytes.len());
        }
        start_process();
...
pub(crate) unsafe fn from_buffered_identity(bytes: &[u8]) -> io::Result<GuestLog> {
    let mut log = unsafe { from_identity(bytes) }?;
    log.1 = LogVersion::Buffered;
    Ok(log)
}

pub(crate) unsafe fn from_ordered_identity(bytes: &[u8]) -> io::Result<GuestLog> {
    let mut log = unsafe { from_identity(bytes) }?;
    log.1 = LogVersion::Ordered;
    Ok(log)
}

pub fn collect(stream: UnixStream, limit: usize, exited: &AtomicBool) -> CapturedGuestLog {
...
mod tests {
    ...
    fn broken_transport_exits_even_with_closed_stderr() {
        if std::env::var_os("LITEINST_LOG_BROKEN_TEST").is_some() {
            ...
            let _writer =
                unsafe { GuestLog(sender.into(), LogVersion::Packets).install() }.unwrap();
