...
pub enum LiteinstDispatchPath {
    ...
    InGuestNestedSigsys,
    /// A cache-line-straddling site that could not be patched.
    CachelineStraddlerFallback,
    /// An unpatchable or otherwise rejected site.
    UnpatchableOrOtherFallback,
    /// A patched-site callback dispatched directly to the in-guest Tool.
    DirectHook,
}
...
impl fmt::Display for LiteinstBackendStatsSnapshot {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "LiteInst instrumentation stats: process_reports={} distinct_rips_patched={} patch_candidates={} decisions[direct_pun={},relocated={},straddler_fallback={},other_fallback={}] paths[in_guest_sigsys={},in_guest_nested_sigsys={},cacheline_straddler={},unpatchable_or_other={},direct_hook={}] classified_candidates={} cacheline_straddlers={} non_straddling={} instruction_lengths[",
            self.process_reports,
...
pub struct LiteinstProcessSiteStats {
    pub(crate) rip: u64,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct LiteinstProcessStats {
...
pub struct LiteinstStatsGlobal {
...
impl LiteinstStatsGlobal {
    pub fn into_source(self) -> LiteinstBackendStatsSource {
        ...
        let mut paths = [0_u64; 5];

        for (process_identity, process) in processes {
            reported_processes.insert(process_identity);
            paths[0] += process.paths[IN_GUEST_SIGSYS];
            paths[1] += process.paths[IN_GUEST_NESTED_SIGSYS];
            paths[2] += process.paths[IN_GUEST_STRADDLER_FALLBACK];
            paths[3] += process.paths[IN_GUEST_OTHER_FALLBACK];
            paths[4] += process.paths[IN_GUEST_DIRECT_HOOK];
        ...
        LiteinstBackendStatsSource {
            snapshot: LiteinstBackendStatsSnapshot {
                ...
                dispatch_paths: CounterSnapshot::new([
                    (LiteinstDispatchPath::InGuestSigsys, paths[0]),
                    (LiteinstDispatchPath::InGuestNestedSigsys, paths[1]),
...
mod tests {
    ...
    fn display_is_deterministic_and_contains_no_raw_identity() {
        ...
        let source = LiteinstBackendStatsSource {
            snapshot: LiteinstBackendStatsSnapshot {
                ...
                dispatch_paths: CounterSnapshot::new([(LiteinstDispatchPath::DirectHook, 9)]),
            },
        };
        ...
        assert!(rendered.contains("paths[in_guest_sigsys=0"));
    ...
    async fn aggregates_equal_rips_from_distinct_fork_processes_and_exact_hits() {
        ...
        assert_eq!(source.dispatch_path_counts(), [2, 4, 6, 8, 18]);
