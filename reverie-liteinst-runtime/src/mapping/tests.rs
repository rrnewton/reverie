...
fn stack_anonymous_request_preserves_provider_args_and_ownership() {
    ...
    let mut owner = owner(&model, vec![0x1000..0x2000]);
...
fn guard_native_install_remove_preserves_all_args_and_updates_views() {
    ...
    model.guard_after = vec![0x2000..0x3000];
    let mut owner = owner(&model, vec![0x1000..0x5000]);
    ...
    assert_eq!(owner.guards.ranges(), &[0x2000..0x3000]);
...
fn guard_partial_native_errors_publish_actual_markers_not_rollback() {
    for advice in [102, 103] {
        ...
        model.guard_after = vec![0x3000..0x4000];
        let mut owner = owner(&model, vec![0x1000..0x5000]);
        ...
        assert_eq!(model.calls, vec![(libc::SYS_madvise, args)]);
        assert_eq!(owner.guards.ranges(), &[0x3000..0x4000]);
        assert_eq!(
            owner.view().unwrap().writable,
...
fn guard_unexpected_baseline_outside_changes_and_false_success_refuse() {
    for scenario in 0..4 {
        ...
        let mut model = Model::new(maps.clone(), maps, 0);
        match scenario {
            0 => model.guard_before = vec![0x2000..0x3000],
            1 => model.guard_after = vec![0x2000..0x4000],
            2 => {}
            _ => model.result = 1,
        }
        let mut owner = owner(&model, vec![0x1000..0x5000]);
...
fn guard_private_pins_unknown_advice_and_generation_stop_before_effect() {
    for advice in [102, 103] {
        for pinned in [false, true] {
            ...
            let pins = if pinned { vec![0x2000..0x3000] } else { vec![] };
            assert!(
                owner
                    .execute(
                        &mut model,
...
fn guard_existing_mapping_transactions_reconcile_observed_markers() {
    ...
    for (number, args, result, after, guards) in cases {
        ...
        model.guard_before = vec![0x2000..0x3000];
...
fn guard_brk_shrink_and_failed_mapping_keep_actual_inventory() {
    ...
    model.guard_before = vec![0x92000..0x93000];
    let mut owner = owner(&model, vec![0x90000..0x93000]);
    ...
    let mut owner = super::tests::owner(&model, vec![0x1000..0x5000]);
    owner.guards = guard::State::Observed(model.guard_before.clone());
    assert_eq!(
        owner
            .execute(
...
fn droppable_preserves_overlap_protection_and_poison_guards() {
    for (guest, pinned, protection, extra, poisoned) in [
        ...
        (vec![0x1000..0x2000], vec![], 7, 0, false),
        (vec![0x1000..0x2000], vec![], 3, libc::MAP_GROWSDOWN, false),
        (vec![0x1000..0x2000], vec![], 3, 0, true),
    ] {
        let maps = vec![map(0x1000, 0x2000, 3)];
...
fn private_and_active_ranges_refuse_before_effect() {
    for (guest, pinned) in [
        (vec![0x1000..0x2000], vec![]),
        (vec![0x1000..0x4000], vec![0x2000..0x3000]),
    ] {
        for number in [
            libc::SYS_munmap,
            libc::SYS_mprotect,
...
fn errors_reconcile_unmap_holes_instead_of_rolling_back() {
    ...
    let mut owner = owner(&model, vec![0x1000..0x4000]);
    assert_eq!(
        owner
            .execute(
                &mut model,
...
fn actual_result_drives_move_and_old_range_retirement() {
    ...
    let mut owner = owner(&model, vec![0x1000..0x3000]);
    assert_eq!(
        owner
            .execute(
                &mut model,
...
fn resulting_alias_inventory_is_checked_before_unpoisoning() {
    for result in [0, -i64::from(libc::ENOMEM)] {
        for guest in [vec![0x1000..0x2000], vec![0x3000..0x4000]] {
...
fn ordinary_host_guard_install_remove_and_effective_views() {
    ...
    for (advice, expected) in [(103, vec![]), (102, vec![page..page + PAGE]), (103, vec![])] {
...
fn ordinary_host_guard_mprotect_move_and_unmap_readback() {
    ...
    assert_eq!(native_guard_readback(&owner), [source..source + PAGE]);
    assert_eq!(
        native_guard_effect(
            &mut owner,
            &mut provider,
