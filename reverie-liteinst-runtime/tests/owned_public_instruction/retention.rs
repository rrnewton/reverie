...
/// Names an optional external root. When absent, the harness creates and keeps
...
/// Return the retained root shared by every case in this test process.
pub fn root() -> std::io::Result<PathBuf> {
    if let Some(root) = std::env::var_os(ARTIFACTS_ENV) {
    ...
    Ok(DEFAULT_ROOT
        .get_or_init(|| {
            tempfile::Builder::new()
                .prefix("reverie-owned-public-instruction-")
                .tempdir()
...
impl Retained {
    ...
    /// Create under the explicit root or the process's retained default root.
    pub fn new(case: &str) -> std::io::Result<Self> {
        Self::under(&root()?, case)
    }
