...
//!   spawns the guest and server roles. It is compiled and linked only. It is not
//!   authorized to run before a separate native preflight, and it carries no
//!   `#[ignore]`: the gate builds it with `--no-run` rather than skipping it.
...
//!
//! `cargo test` injects `LD_LIBRARY_PATH` into this process. That is a real
//! property of the build and host lane, and it is recorded rather than removed:
//! [`selectors`] **refuses** a conflicting variable and never unsets one. A future
//! native attempt must therefore not go through `cargo test`. It must invoke the
//! frozen prebuilt harness binary directly, from an environment that is already
//! clean, and the guard is what checks that at each spawn boundary.
...
const SYMBOLS_ENV: &str = "OWNED_PUBLIC_INSTRUCTION_SYMBOLS";
const DISASM_ENV: &str = "OWNED_PUBLIC_INSTRUCTION_DISASM";
const SECTIONS_ENV: &str = "OWNED_PUBLIC_INSTRUCTION_SECTIONS";
const RELOCS_ENV: &str = "OWNED_PUBLIC_INSTRUCTION_RELOCS";
const RUNTIME_SRC_ENV: &str = "OWNED_PUBLIC_INSTRUCTION_RUNTIME_SRC";
...
fn guard_before_spawn(retained: &Retained, role: &str) {
    let refused = selectors::refusals(selectors::current_names());
    ...
    retained
        .record(
            ...
            report
                .as_deref()
                .unwrap_or("no constructor, runtime or loader selection present\n"),
        )
...
fn run(retained: &Retained) -> Output {
    ...
    guard_before_spawn(retained, "server");
    server.child = Some(
        Command::new(GUEST)
            .args(["server"])
            .arg(&socket)
    ...
    guard_before_spawn(retained, "guest");
...
fn read_required(variable: &str) -> String {
    let path = std::env::var_os(variable)
        .unwrap_or_else(|| panic!("{variable} must name a retained audit input file"));
    std::fs::read_to_string(Path::new(&path))
        .unwrap_or_else(|error| panic!("{variable}: cannot read {path:?}: {error}"))
}

fn bound_body() -> (Vec<linked_audit::Symbol>, Vec<linked_audit::Instruction>) {
    (
        linked_audit::parse_symbols(&read_required(SYMBOLS_ENV)),
        linked_audit::parse_disassembly(&read_required(DISASM_ENV)),
    )
}
...
/// and retains the unselected early-return branch it relies on.
#[test]
fn hostcontrol_default_constructor_is_bound_by_the_linked_init_array() {
    ...
    let symbols = linked_audit::parse_symbols(&read_required(SYMBOLS_ENV));
    let sections = linked_audit::parse_sections(&read_required(SECTIONS_ENV));
    let relocations = linked_audit::parse_relative_relocations(&read_required(RELOCS_ENV));
    ...
    let runtime = read_required(RUNTIME_SRC_ENV);
    let start = runtime
        .find("pub(crate) fn initialize_from_environment()")
        .expect("initialize_from_environment must exist in the pinned runtime source");
    let excerpt: String = runtime[start..]
        .lines()
        .take(20)
        .collect::<Vec<_>>()
        .join("\n");
    assert!(
        excerpt.contains("HOST_RUNTIME_ENV"),
        "the host-runtime selector must be visible: {excerpt}"
    );
    assert!(
        excerpt.contains("REVERIE_LITEINST_TOOL"),
        "the tool selector must be visible: {excerpt}"
    );
    assert!(
        excerpt.contains("None => return Ok(())"),
        "the unselected early return must be present: {excerpt}"
    );
    retained
...
#[test]
fn hostcontrol_selector_guard_refuses_constructor_and_loader_selection() {
    ...
    retained
        ...
        .unwrap();
    assert!(
        live.iter().all(|entry| entry.name.starts_with("LD_")),
