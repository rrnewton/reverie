...
pub const TASK_PANIC_MARKER: &str = "HERMIT_TASK_PANIC";

/// Exit status used when a guest-thread task panics.
///
/// Deliberately not 1: a guest that exits 1 is an ordinary outcome, and a
/// tracer that panicked must be distinguishable from it by exit code alone.
/// 101 is rustc's own convention for a process killed by a panic.
pub const TASK_PANIC_EXIT_CODE: i32 = 101;
