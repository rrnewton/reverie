    ...
    for (source, target) in direct_targets {
        if target < load_end && !canonical_heads.contains(&target) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                format!(
                    "current vDSO direct branch target is not a canonical decoded instruction head: source={source:#x} target={target:#x}"
                ),
            )
            .into());
        }
...
fn plan_stopped_legacy_patches(
    ...
    for (name, bytes, sysno) in VDSO_SYMBOLS {
        ...
        let Some(symbol) = symbols.next() else {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                format!("target vDSO lacks subscribed function {name}"),
            )
            .into());
        };
...
mod tests {
    ...
    #[cfg(target_arch = "x86_64")]
    const LEGACY_FIXTURE_HASH: usize = 0x180;
    #[cfg(target_arch = "x86_64")]
    ...
    #[cfg(target_arch = "x86_64")]
    const LEGACY_FIXTURE_DYNAMIC: usize = 0x700;
    ...
    fn legacy_stopped_fixture() -> Vec<u8> {
        ...
        put_u64(&mut image, 96, IMAGE_LEN as u64);
        put_u64(&mut image, 104, IMAGE_LEN as u64);
        ...
        for (index, (tag, value)) in [
            ...
            (goblin::elf::dynamic::DT_HASH, LEGACY_FIXTURE_HASH as u64),
            (goblin::elf::dynamic::DT_NULL, 0),
        ]
        .into_iter()
        .enumerate()
        {
            let offset = LEGACY_FIXTURE_DYNAMIC + index * 16;
            put_u64(&mut image, offset, tag);
            put_u64(&mut image, offset + 8, value);
        }
        put_u32(&mut image, LEGACY_FIXTURE_HASH, 1);
        put_u32(&mut image, LEGACY_FIXTURE_HASH + 4, DYNSYM_ENTRIES as u32);
        put_u32(&mut image, LEGACY_FIXTURE_HASH + 8, 1);
    ...
    #[cfg(target_arch = "x86_64")]
    #[test]
    fn stopped_legacy_patch_rejects_a_missing_subscribed_function() {
        let mut image = legacy_stopped_fixture();
        put_u32(&mut image, LEGACY_FIXTURE_DYNSYM + 24, 13);

        assert!(
            legacy_stopped_error(&image)
                .contains("target vDSO lacks subscribed function __vdso_time")
        );
    }
