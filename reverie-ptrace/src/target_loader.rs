...
pub(crate) use ordinary::TargetIsolatedRxPage;
pub use ordinary::resolve_host_initializer;
pub(crate) use ordinary::resolve_host_initializer_with_isolated_page;
pub(crate) use ordinary::validate_host_runtime_elf;
        ...
        let mapping = self
            ...
            .ok_or_else(|| {
                io::Error::new(
                    ...
                    format!("target address {address:#x} is unmapped"),
                )
            })?;
        if !mapping.read {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "target address {address:#x} is in non-readable mapping [{:#x}, {:#x})",
...
fn validate_loads(headers: &[goblin::elf::ProgramHeader]) -> io::Result<()> {
    ...
    for p in headers.iter().filter(|p| p.p_type == ph::PT_LOAD) {
        ...
        if p.p_filesz > p.p_memsz
            || p.p_flags & !7 != 0
            || p.p_flags & ph::PF_W == 0 && p.p_flags & ph::PF_R == 0
            || p.p_vaddr % LOAD_PAGE != p.p_offset % LOAD_PAGE
            || (p.p_align > 1
        ...
        let page_start = p.p_vaddr & !(LOAD_PAGE - 1);
        let page_end = end
            .checked_add(LOAD_PAGE - 1)
            .map(|end| end & !(LOAD_PAGE - 1))
            .ok_or_else(|| invalid("ELF load page range overflow"))?;
        let file_memory_end = add(p.p_vaddr, p.p_filesz)?;
        let file_page_end = if p.p_filesz == 0 {
            page_start
        } else {
            file_memory_end
                .checked_add(LOAD_PAGE - 1)
        ...
        let file_page_start = p.p_offset & !(LOAD_PAGE - 1);
            ...
            &rounded_loads
        {
            ...
            if prior_flags != p.p_flags
                || prior_file_overlap_end != file_overlap_end
                || !file_projection_matches
            {
                return Err(invalid("incompatible ELF load page mappings"));
            }
        }
        if loads
            .iter()
