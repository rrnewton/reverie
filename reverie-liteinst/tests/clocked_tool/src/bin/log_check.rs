    ...
    pub fn run() {
        ...
        let result = runtime.block_on(async {
            let mut future: std::pin::Pin<Box<dyn std::future::Future<Output = _>>> = if inherited {
                Box::pin(LiteinstBackend::run_with_inherited_stdio_and_preload_data_and_log_sink::<ClockTool>(command, (), &args[0], data, sink))
            } else {
                Box::pin(LiteinstBackend::run_with_output_and_preload_data_and_log_sink::<ClockTool>(command, (), &args[0], data, sink))
            };
