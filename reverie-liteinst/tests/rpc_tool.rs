...
fn installed_hook_reentry_bypasses_tool_with_shared_coordinator_rpc() {
    ...
    assert!(
        stdout.starts_with(
            "calls=32 traps=1 hooks=32 rpc=34 nested_traps=1 nested_hooks=33 mask_traps=1 mask_hooks=33 mask_result=-1 first_use_exec_result=-95 first_use_signal_result=-1 "
        ),
        "{stdout}"
    );
}
