...
- One coordinator connection and one process are supported by
  ...
  not implemented.
- Syscalls are hosted, and the supported single-process path routes thread and
  process exit callbacks. Signal, CPUID, RDTSC/RDTSCP, and RDRAND/RDSEED events
  are not routed yet.
- Tool mode resets callable signal dispositions before activation, rejects
  ...
  unsupported.
- Timer arming returns success and clock reads return a stable zero, but no RCB
  timer or preemption event is delivered. This supports the single-thread L0
  smoke path; it is not strict scheduling or an L1/L2 determinism claim.
