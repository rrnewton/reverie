...
{
    fn dispatch(&self, event: &mut SyscallEvent) {
        let _allocation_scope = patch_alloc::enter();
...
impl<T: Tool> Guest<T> for LiteinstGuest<'_, T> {
    ...
    async fn tail_inject<S: SyscallInfo>(&mut self, syscall: S) -> Never {
        ...
        let number = number.id() as i64;
        if matches!(number, libc::SYS_exit | libc::SYS_exit_group) {
    ...
    fn set_timer(&mut self, _sched: TimerSchedule) -> Result<(), Error> {
        // Syscall-only L0 runs have no RCB source. No preemption is delivered.
        Ok(())
    }

    fn set_timer_precise(&mut self, _sched: TimerSchedule) -> Result<(), Error> {
        // Exact RCB delivery is not implemented by the LiteInst syscall path.
        Ok(())
    }
...
impl Stack for LocalStack {
    ...
    fn commit(self) -> Result<Self::StackGuard, Errno> {
        Ok(LocalStackGuard { _arena: self.arena })
    }
}
