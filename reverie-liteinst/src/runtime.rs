...
pub(crate) fn prepare_guest_signal_state(
    ...
    let runtime_signals = reverie_preload::signal::runtime_signal_mask();
    if crate::syscall_mode::sud_only() && previous_mask & (sigsys | runtime_signals) != 0 {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
...
fn forward_nested_tool_syscall(event: &mut SyscallEvent) {
    ...
    if unsupported_process {
        ...
        event.result = if crate::syscall_mode::sud_only() {
            guarded_scoped_syscall(event.number, event.args)
        } else {
            guarded_raw_syscall(event.number, event.args)
        };
...
fn guarded_syscall_with(
    ...
    match crate::protected_fd::batch_args(
        ...
        Ok(args) => unsafe { forward(number, args) },
        Err(error) => error,
    }
}
