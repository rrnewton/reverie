...
impl RunObserver {
    ...
    pub(super) fn dropped(&self, caller: bool) {
        ...
        if caller {
            ...
            if state.completion == RunCompletion::Pending {
                state.completion = RunCompletion::Interrupted;
            }
            if state.stdout.state == StreamState::Reading {
                state.stdout.state = StreamState::Interrupted;
            }
            if state.stderr.state == StreamState::Reading {
                state.stderr.state = StreamState::Interrupted;
            }
    ...
    pub(super) fn finished(&self, error: Option<&super::Error>) {
        ...
        let mut state = self.state.lock().unwrap();
        if !state.caller_cancelled {
            state.completion = match error {
    ...
    pub(super) fn stream_state(&self, stream: Stream, value: StreamState) {
        let mut state = self.state.lock().unwrap();
        let caller_cancelled = state.caller_cancelled;
        let stream = Self::stream(&mut state, stream);
        if caller_cancelled
            && (stream.state == StreamState::Interrupted || value == StreamState::Reading)
        {
