...
fn reap_test_child(pid: u32) {
    ...
    loop {
        ...
        assert!(
            ...
            "ordinary child not killed after runtime drop"
        );
...
fn ordinary_child_prefix_observed_before_cancellation_survives_runtime_drop() {
        ...
        let before = tokio::time::timeout(Duration::from_secs(2), async {
            loop {
                ...
                if let Ok(snapshot) = observer.try_snapshot() {
                    if snapshot.stdout.bytes() == b"out\0prefix"
    ...
    assert!(after.wait_status.is_none());
    assert!(!handle.snapshot().root_reaped);
    reap_test_child(after.pid.unwrap());
...
#[test]
fn absent_runtime_does_not_claim_worker_submission_or_child_spawn() {
    let (sink, _handle) = retained_log(Options::bounded(1024));
    let (observer, future) = prepare_command::<()>(
        || Ok(std::process::Command::new("/bin/true")),
        (),
        Vec::new(),
        sink,
        StdioMode::Captured,
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        Box::pin(future).as_mut().poll(&mut context)
    }));
    assert!(result.is_err());
    let snapshot = observer.try_snapshot().unwrap();
