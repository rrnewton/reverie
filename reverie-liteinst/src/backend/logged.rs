...
fn prepare_command<T: Tool + 'static>(
    ...
    let prepared = PreparedCommand::new(command().expect("test command construction"), ());
    LiteinstBackend::prepare_with_owned_command_data_and_log_sink::<T, _>(
...
async fn execute_with_launch<T: Tool + 'static>(
    ...
    launch: &mut dyn owned::LaunchLifetime,
    ...
    .await;
    match result {
        Ok(output) => Ok((output, unwrap_global_after_connections(global).await?)),
        Err(error) => {
            evidence.run.failed(&error);
            if std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| drop(global))).is_err() {
                let cleanup = io::Error::other("global state cleanup unwound");
                evidence.run.cleanup_unwound(&cleanup);
                handle.issue(IssueKind::Cleanup, &cleanup);
            }
            Err(error)
        }
    }
...
unsafe fn inherit_log_fds(bootstrap_fd: i32, guest_fd: i32) -> io::Result<()> {
    for (fd, message) in [
        (
            bootstrap_fd,
            b"liteinst pre_exec log: fcntl F_SETFD bootstrap failed\n".as_slice(),
        ),
        (
            guest_fd,
            b"liteinst pre_exec log: fcntl F_SETFD guest log failed\n".as_slice(),
        ),
    ] {
        unsafe {
...
mod tests {
        ...
        for bad_bootstrap in [false, true] {
            for closed_stderr in [false, true] {
                ...
                let expected = if closed_stderr {
                    ""
                } else if bad_bootstrap {
                    "liteinst pre_exec log: fcntl F_SETFD bootstrap failed\n"
                } else {
                    "liteinst pre_exec log: fcntl F_SETFD guest log failed\n"
                };
    ...
    struct CollectorRuntimeConfig {
        #[serde(skip)]
        runtime: Arc<Mutex<Option<tokio::runtime::Id>>>,
        #[serde(skip)]
        resume: Arc<tokio::sync::Notify>,
        #[serde(skip)]
        finished: Arc<Mutex<Option<std::sync::mpsc::Sender<()>>>>,
    }

    #[derive(Debug, Default)]
    struct CollectorRuntimeGlobal(Option<std::sync::mpsc::Sender<()>>);

    impl Drop for CollectorRuntimeGlobal {
        fn drop(&mut self) {
    ...
    #[reverie::global_tool]
    impl GlobalTool for CollectorRuntimeGlobal {
        type Request = ();
        type Response = ();
        type Config = CollectorRuntimeConfig;

        async fn init_global_state(config: &CollectorRuntimeConfig) -> Self {
            *config.runtime.lock().unwrap() = Some(tokio::runtime::Handle::current().id());
            config.resume.notified().await;
            Self(config.finished.lock().unwrap().take())
        }
    ...
    #[derive(Clone, Default, serde::Serialize, serde::Deserialize)]
    struct CollectorRuntimeTool;

    #[reverie::tool]
    impl Tool for CollectorRuntimeTool {
        type GlobalState = CollectorRuntimeGlobal;
        type ThreadState = ();
    }
    ...
    fn saturated_launcher(inherited: bool, cancel: bool) {
        ...
        let collector_runtime = tokio::runtime::Builder::new_current_thread()
        ...
        collector_runtime.spawn_blocking(move || {
            entered.send(()).unwrap();
            gate.recv_timeout(Duration::from_secs(10)).unwrap();
            finished.send(()).unwrap();
        });
        entry.recv_timeout(Duration::from_secs(2)).unwrap();
        let collector_runtime_id = collector_runtime.handle().id();
        owned::TEST_RUNTIME.with(|runtime| {
            assert!(runtime.borrow_mut().replace(collector_runtime).is_none());
        });
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
        ...
        let (sink, handle) = retained_log(Options::bounded(1024));
        let config = CollectorRuntimeConfig::default();
        let (finished, launch_finished) = mpsc::channel();
        *config.finished.lock().unwrap() = Some(finished);
        runtime.block_on(async {
            let mut future: std::pin::Pin<Box<dyn std::future::Future<Output = _>>> = if inherited {
                Box::pin(LiteinstBackend::run_with_inherited_stdio_and_preload_data_and_log_sink::<CollectorRuntimeTool>(command, config.clone(), "/bin/true", Vec::new(), sink))
            } else {
                Box::pin(LiteinstBackend::run_with_output_and_preload_data_and_log_sink::<CollectorRuntimeTool>(command, config.clone(), "/bin/true", Vec::new(), sink))
            };
            tokio::time::timeout(Duration::from_secs(2), async {
                tokio::select! {
                    result = &mut future => panic!("launcher returned before initialization: {result:?}"),
                    _ = async { while config.runtime.lock().unwrap().is_none() { tokio::task::yield_now().await; } } => {},
                }
            }).await.unwrap();
            assert_eq!(*config.runtime.lock().unwrap(), Some(collector_runtime_id), "the saturated pool must belong to the collector runtime");
            config.resume.notify_one();
            tokio::time::timeout(Duration::from_secs(2), async {
                tokio::select! {
        ...
        assert!(handle.snapshot().streams.is_empty());
        if cancel {
            assert_eq!(handle.snapshot().reader, ReaderState::Queued);
        }
